...
**Content type namespaces:** Shared components that query content types must use the registered ClassName namespace from Entities (`CONTENT_TYPE_NAME` / C# `namespace`), not the .NET project name if they differ. See [content-type-namespaces.md](../_shared/references/content-type-namespaces.md).
